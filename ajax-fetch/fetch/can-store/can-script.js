	 ...
	 if(request.status === 200 ) {
	   ...
	   products = request.response;
        initialize();	 
		 ...
		 console.log('My error: Network request for products.json failed with response ' + request.status + ': ' + request.statusText);
	 }
}
request.send();
...
function initialize() {
  ...
  */
  ...
  function fetchBlob (product) {
	  ...
	  secRequest.open('GET', url);
	  secRequest.responseType = 'blob';
	  
	  secRequest.onload = function() {
		 // alert(secRequest.status);
	      if(secRequest.status === 200 ) {
	       // alert(' Images -- >Loaded');
			objectURL = URL.createObjectURL(secRequest.response);
			showProduct(objectURL, product);
	      }
	      else {
		        console.log('Network request for "' + product.name + '" image failed with response ' + secRequest.status + ': ' + secRequest.statusText);
	      }
